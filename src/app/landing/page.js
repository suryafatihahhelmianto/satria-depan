...
export default function Dashboard() {
  ...
  const slides = [
    { id: 1, src: '/img/login.png', title: 'Peningkatan layanan kesehatan di daerah terpencil' },
    { id: 2, src: '/img/dashboard.png', title: 'Pemulihan ekonomi pasca-pandemi' },
    { id: 3, src: '/img/dashboard1.png', title: 'Peran pendidikan dalam membangun masa depan' },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <header className="bg-ijoTebu text-white text-center py-4">
        ...
        <h2 className="text-4xl font-bold mt-10">"Sistem Analitik dan Teknologi Rajawali-IPB untuk Inovasi Agro: Pengukuran Kinerja Rantai Pasok dan Prediksi Rendemen"</h2>
      ...
      <div className="flex justify-center items-center mt-10 space-x-10 px-5">
        {/* Slider Section */}
        <div className="relative w-3/4 h-[600px] overflow-hidden rounded-lg">  {/* Perbesar slider */}
          <div className="slides flex transition-transform duration-700 ease-in-out h-full" style={{ transform: `translateX(-${currentSlideIndex * 100}%)` }}>
            {slides.map((slide) => (
              <div key={slide.id} className="min-w-full h-full relative">
                <div className="absolute inset-0"> {/* Ensure full coverage */}
                  <Image
                    src={slide.src}
                    alt={`Slide ${slide.id}`}
                    fill
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2 z-20">
            {slides.map((_, index) => (
              <span
                key={index}
                className={`dot w-3 h-3 bg-white bg-opacity-50 rounded-full cursor-pointer ${index === currentSlideIndex ? 'bg-ijoTebu' : ''}`}
        ...
        {/* Card Section */}
        <div className="bg-gray-300 p-4 rounded-lg w-1/4 h-80 flex flex-col justify-center items-center space-y-4"> {/* Perkecil card */}
          <h2 className="text-lg font-bold">Coba Sekarang</h2>
          <Link href="/demo" className="bg-ijoTebu text-white font-semibold py-2 px-4 rounded-lg hover:bg-green-700">
            Demo Fitur
          </Link>
        </div>
      </div>
